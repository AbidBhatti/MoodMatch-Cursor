...
class GameState: ObservableObject {
    ...
    /// Resets all game state properties to their initial values, preparing for a new game session.
    func reset() {
        score = 0
        lives = Self.maxLives
    ...
    /// Initializes a new game by resetting all state properties and marking the game as active.
    func startGame() {
        reset()
        isGameActive = true
    }
    
    /// Marks the game as ended by updating the active and game over status flags.
    func endGame() {
        isGameActive = false
        isGameOver = true
    }
    ...
    /// Handles an incorrect match by decrementing lives, updating the last result, and displaying the result overlay.
    ...
    /// Resets the remaining time to the maximum allowed for a round.
    func resetTimer() {
        timeRemaining = Self.maxTime
    }
} 